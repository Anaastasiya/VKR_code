...
function runScript() {
  ...
  function CorrectPrices() {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (price == "Неизвестно") {
        cells[8].textContent =
          (1000 + Math.random() * 5000).toFixed(2) + " RUB"; // случайная цена от 1000 до 6000
      }
    }
  }
  ...
  function filterRoutes() {
      ...
      if (from !== fromCity || to !== toCity) {
        ...
        if (optimalRoute) {
          for (let j = 0; j < optimalRoutes.length; j++) {
            ...
            if (
              optimalRoute.number === optimalRoutes[j].number &&
              optimalRoute.from === optimalRoutes[j].from &&
              optimalRoute.to === optimalRoutes[j].to
            ) {
              duplicate = true;
              break;
            }
        ...
        // Если условие выполняется, то заменить оптимальный рейс на текущий
        if (
          (k_t >= 1 && k_p <= 1) ||
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      for (let j = 0; j < optimalRoutes.length; j++) {
        // Если номер, город отправления и город прибытия совпадают, то считать текущий рейс оптимальным и выйти из цикла
        if (
          number === optimalRoutes[j].number &&
          from === optimalRoutes[j].from &&
          to === optimalRoutes[j].to
        ) {
          isOptimal = true;
  ...
  function getCoordinates(city, selectedRegion) {
    ...
    stations = stationsData.countries
      .flatMap((country) => country.regions)
      .find((region) => region.title === selectedRegion)
      .settlements.find((settlement) => settlement.title === city).stations;
  ...
  function showRoute() {
    ...
    ymaps.ready(function () {
      ...
      for (var t = 0; t < route.length; t++) {
        //найти левую нижнюю и правую верхнюю точки
      ...
      for (var t = 0; t < route.length - 1; t++) {
        if (transport_types[t] != "plane") {
          ...
          urls.push(
            "https://geocode-maps.yandex.ru/1.x/?apikey=9431bf30-a1d4-4319-88e3-b17a8ad8830b&format=json&geocode=" +
              route[t][1] +
              "," +
              route[t][0]
          );
          ...
          Promise.all(urls.map((u) => fetch(u)))
            ...
            .then(function (data) {
              ...
              for (var i = 0; i < data.length; i++) {
                cities.push(
                  data[i]["response"]["GeoObjectCollection"][
                    "featureMember"
              ...
              ymaps
                .route(cities, (options = { hasBalloon: false }))
  ...
  function LeaveOnlyRouteInTable(route) {
    ...
    // Получить все строки таблицы
    let rows = table.querySelectorAll("tr");
    //сделать все строки таблицы невидимыми
    transport_types = [];
    for (let i = 1; i < rows.length; i++) {
    ...
    for (let r = 0; r < route.length - 1; r++) {
      ...
      for (let i = 0; i < cities_coords.length; i++) {
        if (
          cities_coords[i]["coords"][0] === lat1 &&
          cities_coords[i]["coords"][1] === lon1
        ) {
          city1 = cities_coords[i]["city"];
        }
        if (
          cities_coords[i]["coords"][0] === lat2 &&
          cities_coords[i]["coords"][1] === lon2
        ) {
      ...
      for (let j = 0; j < optimalRoutes.length; j++) {
        if (optimalRoutes[j].from == city1 && optimalRoutes[j].to == city2) {
          for (let i = 1; i < rows.length; i++) {
            let cells = rows[i].querySelectorAll("td");
            if (
              optimalRoutes[j].number == cells[1].textContent &&
              optimalRoutes[j].type == cells[2].textContent &&
              optimalRoutes[j].from + "—" + optimalRoutes[j].to ==
                cells[0].textContent &&
